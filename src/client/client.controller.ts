...
export class ClientController {
    ...
    @Get()
    getClient(
    @Query() mesQueryParams: GetPaginatedClientDto,
    ) {}
 
    @Post()
    addClientt(
        @Body() newClient: AddClientDto
        ){
    ...
    @Get('/:idClient')
        getClientById(
            ...
            const client= this.clients.find((actualClient)=> actualClient.idClient===+idClient);
            if(client)
                return client;
            else 
                throw new NotFoundException(`le client d'id ${idClient} n'existe pas`);
    ...
    updateClient(
        ...
        client.description= newClient.description ? newClient.description : client.description;
        return (client);    
    }
